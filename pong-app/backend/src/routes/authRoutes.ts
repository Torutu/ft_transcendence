...
import env from '../env';
import { sendVerificationCode, sendPasswordResetEmail, sendRegisterSuccessEmail } from '../service/emailService';
...
export default function authRoutes(app: FastifyInstance, options: AuthRoutesOptions) {
  ...
  // 2FA functions
  const generateTwoFactorSecret = (email: string): speakeasy.GeneratedSecret => {
  ...
  const createUserFriendships = async (newUser: { id: number; username: string }) => {
  ...
  const createAndSendVerificationCode = async (
    ...
    await sendVerificationCode(user.email, verificationCode.code, user.username, context);

    return verificationCode;
  };

  // Helper function for verification code validation
  const validateAndConsumeVerificationCode = async (userId: number, code: string) => {
  ...
  const updateUserLoginStatus = async (userId: number, additionalData: any = {}) => {
    ...
    async (request, reply) => {
      ...
      try {
        ...
        if (!user) {
          ...
          await sendRegisterSuccessEmail(payload.email, username, getBaseUrl(request));
        } else {
  ...
  app.post<{ Body: VerifyInput }>(
    ...
    async (request, reply) => {
      ...
      try {
        ...
        // Use helper function for status update
        const additionalData = !user.twoFactorRegistered ? { twoFactorRegistered: true } : {};
   ...
   ************************************************************************ */
  app.post<{ Body: { userId: number; context?: 'email-verification' | 'login-2fa' } }>(
    ...
    if (token) {
      try {
        ...
        if (decoded && decoded.userId) {
          ...
          console.log(`Marked user ${decoded.userId} offline during logout with expired token`);
        }
      } catch (decodeErr) {
...
const cleanupInactiveUsers = async () => {
  ...
  const startupCleanup = async () => {
    try {
      ...
      console.log(`✅ Marked ${result.count} users as offline after server restart`);
