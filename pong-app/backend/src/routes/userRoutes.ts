...
export default function userRoutes(app: FastifyInstance, options: UserRoutesOptions) {
  ...
  app.get('/user/profile', async (request, reply) => {
    try {
      ...
      return reply.status(500).send({ error: 'PROFILE_FETCH_FAILED', message: 'Failed to fetch profile' });
    }
  });

  // PUT /user/profile - Update profile (for MyLockerTab save functionality)
  app.put<{ Body: UpdateProfileInput }>('/user/profile', async (request, reply) => {
    try {
      const decoded = verifyAuth(request);
      ...
      if (dateOfBirth && !validator.isDate(dateOfBirth)) {
        return reply.status(400).send({ error: 'INVALID_DATE', message: 'Invalid date format' });
      }
    ...
    if (profilePic && profilePic.length > 0) {
      ...
      if (profilePic.length > 1.5 * 1024 * 1024) { // 1.5MB base64 = ~1MB image
        return reply.status(400).send({ 
...
const updatedUser = await prisma.user.update({
  ...
  data: {
    ...
    profilePic: profilePic || null
      ...
      return reply.status(500).send({ error: 'PROFILE_UPDATE_FAILED', message: 'Failed to update profile' });
    }
  });
  ...
  app.get('/user/stats', async (request, reply) => {
    try {
      ...
      return reply.status(500).send({ error: 'STATS_FETCH_FAILED', message: 'Failed to fetch statistics' });
    }
  });
  ...
  app.post('/user/avatar', async (request, reply) => {
    try {
      ...
      return reply.status(500).send({ error: 'UPLOAD_FAILED', message: 'Upload failed' });
    }
  });
...
app.get('/user/avatars', async (request, reply) => {
  try {
    ...
    return reply.status(500).send({ error: 'AVATARS_FETCH_FAILED', message: 'Failed to fetch avatars' });
  }
});
