...
for (const game of allGames) {
  try {
    ...
    const userWon = rounds.winner.username === userRecord.username || 
                   rounds.winner.playerId === userId;
                // ||   rounds.userWon === true;
...
monthlyGames.forEach(game => {
  try {
    const rounds = JSON.parse(game.rounds_json || '{}');
    if (rounds.winner.username === userRecord.username || rounds.winner.playerId === userId /*|| rounds.userWon*/) {
      monthlyWins++;
    }
  } catch (error) {
    // Skip invalid JSON
  ...
  try {
    ...
    const processedMatches = recentGames.map(game => {
      ...
      try {
        ...
        //   if (rounds.opponentName) opponent = rounds.opponentName; <-- ??? these don't exist on the saved data ???
          if (rounds.player2 && rounds.player2.username !== userRecord.username) opponent = rounds.player2.username;
          if (rounds.player1 && rounds.player1.username !== userRecord.username) opponent = rounds.player1.username;
          
          // Extract result
          if (rounds.winner.username === userRecord.username) isUserWinner = true;
          if (rounds.winner.playerId === userId) isUserWinner = true;
        //   if (rounds.userWon !== undefined) isUserWinner = rounds.userWon;
          
          // EXTRACT ACTUAL SCORES - FIXED LOGIC        <-- ??? feels unnecessary...
        ...
        //     }
        //   } else 
          if (rounds.player1 && rounds.player2) {
            ...
            const isUserPlayer1 = rounds.player1.username === userRecord.username /*|| 
                                 rounds.player1.playerId === userRecord.username;*/
...
fastify.get('/avatars', async (request: FastifyRequest, reply: FastifyReply) => {
  try {
    // Define available avatars with your friend's descriptions
    const avatars = [
      {
        id: 'AstroAce',
        name: 'Astro Ace', 
